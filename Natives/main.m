...
void init_logDeviceAndVer(char *argument) {
    // PocketJ Launcher version
    NSLog(@"[Pre-Init] PocketJ Launcher INIT!");
...
void init_setupMultiDir() {
    ...
    NSString *multidirPath = multidir.length > 0
        ? [instancesPath stringByAppendingPathComponent:multidir] : nil;
    BOOL isDirectory = NO;
    BOOL hasSelectedInstance = multidirPath.length > 0 &&
        [fm fileExistsAtPath:multidirPath isDirectory:&isDirectory] &&
        isDirectory;
    if (hasSelectedInstance) {
        NSLog(@"[Pre-init] Restored game directory preference (%@)\n",
              multidir);
        [fm createSymbolicLinkAtPath:lasmPath
                withDestinationPath:multidirPath error:nil];
        [fm changeCurrentDirectoryPath:lasmPath];
    } else {
        setPrefObject(@"general.game_directory", @"");
        NSLog(@"[Pre-init] No game instance selected.\n");
...
void init_setupHomeDirectory() {
    ...
    homeDir = [NSString stringWithFormat:@"%s/Documents%@", getenv("HOME"),
        isNotSandboxed ? @"/PocketJLauncher":@""];
...
int main(int argc, char *argv[]) {
    ...
    // If sandbox is disabled, W^X JIT can be enabled by PocketJ Launcher itself
