...
- (void)viewDidLoad {
    ...
    self.searchController.searchBar.placeholder = localize(@"搜索版本", nil);
    ...
    self.searchQueue = dispatch_queue_create("com.Erico.PocketJLauncher.forge.search", DISPATCH_QUEUE_SERIAL);
...
- (void)actionCancelDownload {
    ...
    showDialog(localize(@"下载已取消", nil), localize(@"已停止下载安装器。", nil));
}
...
- (NSString *)getLabelForVersionType:(NSString *)version {
    if ([version containsString:@"recommended"]) {
        return localize(@"推荐版", nil);
    } else if ([version containsString:@"beta"] || [version containsString:@"-beta"]) {
        return localize(@"测试版", nil);
    } else if ([version containsString:@"alpha"] || [version containsString:@"-alpha"]) {
        return localize(@"早期测试版", nil);
    } else {
        return localize(@"正式版", nil);
    }
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (self.isDataLoading) {
        headerView.titleLabel.text = localize(@"正在加载…", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isDataLoading) {
        cell.versionLabel.text = localize(@"正在加载…", nil);
...
- (void)parser:(NSXMLParser *)parser parseErrorOccurred:(NSError *)parseError {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self.refreshControl endRefreshing];
        showDialog(localize(@"加载版本失败", nil), parseError.localizedDescription);
        [self switchToReadyState];
    });
}
