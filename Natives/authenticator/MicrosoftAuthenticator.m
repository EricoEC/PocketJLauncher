...
- (void)acquireXboxProfile:(NSString *)xblUhs xstsToken:(NSString *)xblXsts callback:(Callback)callback {
    ...
    [manager GET:@"https://profile.xboxlive.com/users/me/profile/settings?settings=PublicGamerpic,Gamertag" parameters:nil headers:headers progress:nil success:^(NSURLSessionDataTask *task, NSDictionary *response) {
        id rawSettings = response[@"profileUsers"][0][@"settings"];
        NSArray *settings = [rawSettings isKindOfClass:NSArray.class]
            ? rawSettings : @[];
        NSString *gamerpic = nil;
        NSString *gamertag = nil;
        for (NSDictionary *setting in settings) {
            NSString *identifier = setting[@"id"];
            NSString *value = setting[@"value"];
            if (![value isKindOfClass:NSString.class] || value.length == 0) continue;
            if ([identifier isEqualToString:@"PublicGamerpic"]) gamerpic = value;
            if ([identifier isEqualToString:@"Gamertag"]) gamertag = value;
        }
        NSURLComponents *components = gamerpic.length
            ? [NSURLComponents componentsWithString:gamerpic] : nil;
        if (components.URL) {
            NSMutableArray<NSURLQueryItem *> *query =
                [components.queryItems mutableCopy] ?: [NSMutableArray array];
            [query addObject:[NSURLQueryItem queryItemWithName:@"h" value:@"120"]];
            [query addObject:[NSURLQueryItem queryItemWithName:@"w" value:@"120"]];
            components.queryItems = query;
            // Only a verified new URL may replace the last saved avatar URL.
            self.authData[@"profilePicURL"] = components.URL.absoluteString;
        }
        if (gamertag.length) self.authData[@"xboxGamertag"] = gamertag;
        callback(nil, YES);
    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        callback(error, NO);
    }];
...
- (void)checkMCProfile:(NSString *)mcAccessToken callback:(Callback)callback {
    ...
    [manager GET:@"https://api.minecraftservices.com/minecraft/profile" parameters:nil headers:headers progress:nil success:^(NSURLSessionDataTask *task, NSDictionary *response) {
        ...
        NSArray *skins = response[@"skins"];
        NSString *skinTextureURL =
            [skins.firstObject isKindOfClass:NSDictionary.class]
                ? skins.firstObject[@"url"] : nil;
        if (skinTextureURL.length > 0) {
            self.authData[@"skinTextureURL"] =
                [skinTextureURL stringByReplacingOccurrencesOfString:@"http://"
                                                          withString:@"https://"];
        }
        // Keep profilePicURL from the official Xbox profile response. Do not
        // replace it with a launcher-generated or third-party rendered head.
...
- (void)refreshTokenWithCallback:(Callback)callback {
    ...
    if (!self.tokenData) {
        showDialog(localize(@"Error", nil), localize(@"localization.error.keychain_load", nil));
        callback(nil, YES);
        return;
    }
    ...
    } else if ([self.authData[@"skinTextureURL"] length] == 0) {
        // Upgrade older saved accounts by fetching the official Minecraft skin
        // texture even when their access token has not expired yet.
        self.authData[@"accessToken"] = self.tokenData[@"accessToken"];
        self.authData[@"msaRefreshToken"] = self.tokenData[@"refreshToken"];
        [self checkMCProfile:self.tokenData[@"accessToken"] callback:callback];
    } else {
        callback(nil, YES);
    }
}
...
- (BOOL)saveChanges {
    ...
    if (!savedToKeychain) {
        showDialog(localize(@"Error", nil), localize(@"localization.error.keychain_save", nil));
        return NO;
    }
...
+ (NSDictionary *)tokenDataOfProfile:(NSString *)profile {
    ...
    if (status == errSecSuccess) {
        ...
        NSDictionary *result = [NSKeyedUnarchiver
            unarchivedObjectOfClasses:classes
            fromData:(__bridge NSData *)resultData
            error:&error];
        if (error) {
