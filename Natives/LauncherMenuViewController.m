...
- (void)updateAccountInfo {
    ...
    // Prefer the launcher-owned on-disk Minecraft head cache so the avatar
    // remains available offline and after restarting the app.
    UIImage *placeholder =
        [AccountListViewController cachedAvatarForAccount:selected] ?:
        [UIImage imageNamed:@"DefaultAccount"];
    // Never feed the original 120px network render directly to this custom
    // button: sizeToFit would expand the entire navigation item around it.
    [self.accountButton setImage:placeholder forState:UIControlStateNormal];
    if ([BaseAuthenticator
            storageKeyForAuthData:selected].length > 0) {
        NSString *expectedKey =
            [BaseAuthenticator storageKeyForAuthData:selected];
        [AccountListViewController loadBestAvatarForAccount:selected
            completion:^(UIImage *avatar) {
                NSString *currentKey = BaseAuthenticator.current.authData
                    ? [BaseAuthenticator storageKeyForAuthData:
                        BaseAuthenticator.current.authData] : nil;
                if ([currentKey isEqualToString:expectedKey]) {
                    [self.accountButton setImage:avatar
