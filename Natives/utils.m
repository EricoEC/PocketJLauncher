...
BOOL getEntitlementValue(NSString *key) {
    ...
    BOOL available = ![(__bridge id)value isKindOfClass:NSNumber.class] ||
                     [(__bridge id)value boolValue];
    CFRelease(value);
    return available;
}
...
NSString* localize(NSString* key, NSString* comment) {
    // Ask the main bundle directly so iOS applies this app's language chosen
    // in Settings. Do not compare the result with the key: many Simplified
    // Chinese source keys intentionally translate to the exact same string,
    // and treating equality as a missing translation incorrectly forced the
    // interface back to English.
