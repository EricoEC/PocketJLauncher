...
void init_hookUIKitConstructor(void) {
    ...
    // Wide layout is independent from the iPadOS interface emulation above.
    // Only replace the system size classes when the user explicitly enables it.
    if (getPrefBool(@"debug.debug_wide_ui")) {
        swizzle(UITraitCollection.class, @selector(horizontalSizeClass), @selector(hook_horizontalSizeClass));
        swizzle(UITraitCollection.class, @selector(verticalSizeClass), @selector(hook_verticalSizeClass));
    }
...
- (UIViewController *)visibleViewController {
    ...
    while (current) {
        ...
        if (presented &&
            ![presented isKindOfClass:UIAlertController.class] &&
            ![presented isKindOfClass:NSClassFromString(@"UIInputWindowController")]) {
            current = presented;
            continue;
        }
