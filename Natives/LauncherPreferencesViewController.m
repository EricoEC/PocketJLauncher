...
- (void)viewDidLoad
{
    ...
    self.hasDetail = YES;
    self.prefDetailVisible = self.presentedInGame || self.navigationController == nil;
    ...
    BOOL(^whenNotInGame)() = ^BOOL(){
        return !self.presentedInGame && self.navigationController != nil;
    };
    self.prefContents = @[
        @[
            // General settings
            ...
            @{@"key": @"appicon",
              ...
              @"action": ^void(NSString *iconName) {
                  if ([iconName isEqualToString:@"automatic"] ||
                      [iconName isEqualToString:@"AppIcon-Light"]) {
                      iconName = nil;
                  }
            ...
            @{@"key": @"fluid_button_slide",
                @"title": localize(@"丝滑滑入按键", nil),
                @"detail": localize(@"滑入即触发，跨缝不中断，并支持四向对角移动。切换后立即生效。", nil),
                @"icon": @"hand.draw.fill",
                @"hasDetail": @YES,
                @"type": self.typeSwitch,
                @"action": ^(BOOL enabled) {
    ...
    [super viewDidLoad];
    if (self.presentedInGame || self.navigationController == nil) {
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.presentedInGame || self.navigationController == nil) {
        ...
        if ([presenter respondsToSelector:@selector(updatePreferenceChanges)]) {
            [presenter performSelector:@selector(updatePreferenceChanges)];
        }
    }
}
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section == 0) { // Add to general section
        return [NSString stringWithFormat:@"PocketJ Launcher %@-%s (%s/%s)\n%@ on %@ (%s)\nPID: %d",
