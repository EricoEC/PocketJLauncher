...
- (void)loadControlFile:(NSString *)name {
    ...
    if (self.layoutDictionary[@"NSErrorObject"] != nil) {
        showDialog(localize(@"Error", nil), [NSString stringWithFormat:localize(@"localization.error.open_file_format", nil), controlFilePath, [self.layoutDictionary[@"NSErrorObject"] localizedDescription]]);
        return;
    }
