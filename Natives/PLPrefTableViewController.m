...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.hasDetail && self.navigationController) {
        ...
        self.navigationItem.rightBarButtonItems = accountItem ? @[accountItem, [self drawHelpButton]] : @[[self drawHelpButton]];
    }
...
- (UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cellStyle != UITableViewCellStyleValue1) {
        ...
        if ([item[@"hasDetail"] boolValue] && self.prefDetailVisible) {
            cell.detailTextLabel.text = item[@"detail"] ?: localize(
                ([NSString stringWithFormat:@"preference.detail.%@", key]), nil);
        }
    }
...
- (void)initViewCreation {
    ...
    self.typePickField = ^void(UITableViewCell *cell, NSString *section, NSString *key, NSDictionary *item) {
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        cell.selectionStyle = UITableViewCellSelectionStyleGray;
        id selectedKey = weakSelf.getPreference(section, key);
        NSUInteger selectedIndex = [item[@"pickKeys"] indexOfObject:selectedKey];
        cell.detailTextLabel.text =
            selectedIndex != NSNotFound && selectedIndex < [item[@"pickList"] count]
                ? item[@"pickList"][selectedIndex]
                : selectedKey;
    };
...
- (void)tableView:(UITableView *)tableView openPickerAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([item[@"hasDetail"] boolValue]) {
        message = item[@"detail"] ?: localize(
            ([NSString stringWithFormat:@"preference.detail.%@", item[@"key"]]), nil);
    }
    ...
    for (int i = 0; i < pickList.count; i++) {
        ...
        if ([self.getPreference(self.prefSections[indexPath.section],
                item[@"key"]) isEqual:pickKeys[i]]) {
