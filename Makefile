...
$(warning Building on Linux. Note that all targets may not compile or require external components.)
	...
	$(error This platform is not currently supported for building PocketJ Launcher.)
endif
...
POJAV_BUNDLE_DIR      ?= $(OUTPUTDIR)/PocketJLauncher.app
...
METHOD_PACKAGE = \
	...
	if [ '$(SLIMMED_ONLY)' = '0' ]; then \
		zip --symlinks -r $(OUTPUTDIR)/com.Erico.PocketJLauncher-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX Payload; \
	fi; \
	if [ '$(SLIMMED)' = '1' ] || [ '$(SLIMMED_ONLY)' = '1' ]; then \
		zip --symlinks -r $(OUTPUTDIR)/com.Erico.PocketJLauncher.slimmed-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX Payload --exclude='Payload/PocketJLauncher.app/java_runtimes/*'; \
	fi
...
help:
	echo 'Makefile to compile PocketJ Launcher'
	echo ''
	echo 'Usage:'
	...
	echo '    make payload                        Makes Payload/PocketJLauncher.app'
	echo '    make package                        Builds ipa of PocketJ Launcher'
...
native: dep_mg
	echo '[PocketJ Launcher v$(VERSION)] native - start'
	rm -rf $(WORKINGDIR)/PocketJLauncher.app
	mkdir -p $(WORKINGDIR)
	cd $(WORKINGDIR) && cmake \
	...
	#	--target awt_headless awt_xawt libOSMesaOverride.dylib tinygl4angle PocketJLauncher
	rm $(WORKINGDIR)/libawt_headless.dylib
	echo '[PocketJ Launcher v$(VERSION)] native - end'

java:
	echo '[PocketJ Launcher v$(VERSION)] java - start'
	# JavaApp's per-source rules share build/classes. Parallel javac processes can
	# truncate one another's outputs, producing intermittent empty .class files.
	$(MAKE) -C JavaApp clean
	$(MAKE) -C JavaApp -j1 BOOTJDK=$(BOOTJDK)
	echo '[PocketJ Launcher v$(VERSION)] java - end'

jre: native
	echo '[PocketJ Launcher v$(VERSION)] jre - start'
	...
	echo '[PocketJ Launcher v$(VERSION)] jre - end'
...
dep_mg:
	...
	echo '[PocketJ Launcher v$(VERSION)] dep_mg - end'

assets:
	echo '[PocketJ Launcher v$(VERSION)] assets - start'
	if [ '$(IOS)' = '0' ] && [ '$(DETECTPLAT)' = 'Darwin' ]; then \
		mkdir -p $(WORKINGDIR)/PocketJLauncher.app/Base.lproj; \
		rm -f $(SOURCEDIR)/Natives/resources/Assets.car \
			$(SOURCEDIR)/Natives/resources/AppIcon*.png; \
		xcrun actool $(SOURCEDIR)/Natives/Assets.xcassets \
			--compile $(SOURCEDIR)/Natives/resources \
			--platform iphoneos \
			--minimum-deployment-target 14.0 \
			--app-icon AppIcon-Light \
			--alternate-app-icon AppIcon-White \
			--alternate-app-icon AppIcon-Dark \
			--output-partial-info-plist /dev/null || exit 1; \
	else \
		echo 'Due to the required tools not being available, you cannot compile the extras for PocketJ Launcher with an iOS device.'; \
	fi
	echo '[PocketJ Launcher v$(VERSION)] assets - end'

payload: native dep_mg java jre assets
	echo '[PocketJ Launcher v$(VERSION)] payload - start'
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/PocketJLauncher.app/libs)
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/PocketJLauncher.app/libs_caciocavallo)
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/PocketJLauncher.app/libs_caciocavallo17)
	cp -R $(SOURCEDIR)/Natives/resources/en.lproj/LaunchScreen.storyboardc $(WORKINGDIR)/PocketJLauncher.app/Base.lproj/ || exit 1
	cp -R $(SOURCEDIR)/Natives/resources/* $(WORKINGDIR)/PocketJLauncher.app/ || exit 1
	cp $(WORKINGDIR)/*.dylib $(WORKINGDIR)/PocketJLauncher.app/Frameworks/ || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/others/* $(WORKINGDIR)/PocketJLauncher.app/libs/ || exit 1
	cp $(SOURCEDIR)/JavaApp/build/*.jar $(WORKINGDIR)/PocketJLauncher.app/libs/ || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/caciocavallo/* $(WORKINGDIR)/PocketJLauncher.app/libs_caciocavallo || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/caciocavallo17/* $(WORKINGDIR)/PocketJLauncher.app/libs_caciocavallo17 || exit 1
	$(call METHOD_DIRCHECK,$(OUTPUTDIR)/Payload)
	cp -R $(WORKINGDIR)/PocketJLauncher.app $(OUTPUTDIR)/Payload
	if [ '$(SLIMMED_ONLY)' != '1' ]; then \
		cp -R $(OUTPUTDIR)/java_runtimes $(OUTPUTDIR)/Payload/PocketJLauncher.app; \
	fi
	ldid -S $(OUTPUTDIR)/Payload/PocketJLauncher.app; \
	if [ '$(TROLLSTORE_JIT_ENT)' == '1' ]; then \
		ldid -S$(SOURCEDIR)/entitlements.trollstore.xml $(OUTPUTDIR)/Payload/PocketJLauncher.app/PocketJLauncher; \
	elif [ '$(PLATFORM)' == '6' ]; then \
		ldid -S$(SOURCEDIR)/entitlements.codesign.xml $(OUTPUTDIR)/Payload/PocketJLauncher.app/PocketJLauncher; \
	else \
		ldid -S$(SOURCEDIR)/entitlements.sideload.xml $(OUTPUTDIR)/Payload/PocketJLauncher.app/PocketJLauncher; \
	fi
	chmod -R 755 $(OUTPUTDIR)/Payload
	if [ '$(PLATFORM)' != '2' ]; then \
		$(call METHOD_MACHO,$(OUTPUTDIR)/Payload/PocketJLauncher.app,$(call METHOD_CHANGE_PLAT,$(PLATFORM),$$file)); \
		$(call METHOD_MACHO,$(OUTPUTDIR)/java_runtimes,$(call METHOD_CHANGE_PLAT,$(PLATFORM),$$file)); \
	fi
	echo '[PocketJ Launcher v$(VERSION)] payload - end'

deploy:
	echo '[PocketJ Launcher v$(VERSION)] deploy - start'
	cd $(OUTPUTDIR); \
	if [ '$(IOS)' = '1' ]; then \
		ldid -S $(WORKINGDIR)/PocketJLauncher.app || exit 1; \
		ldid -S$(SOURCEDIR)/entitlements.trollstore.xml $(WORKINGDIR)/PocketJLauncher.app/PocketJLauncher || exit 1; \
		sudo mv $(WORKINGDIR)/*.dylib $(PREFIX)Applications/PocketJLauncher.app/Frameworks/ || exit 1; \
		sudo mv $(WORKINGDIR)/PocketJLauncher.app/PocketJLauncher $(PREFIX)Applications/PocketJLauncher.app/PocketJLauncher || exit 1; \
		sudo mv $(SOURCEDIR)/JavaApp/build/*.jar $(PREFIX)Applications/PocketJLauncher.app/libs/ || exit 1; \
		cd $(PREFIX)Applications/PocketJLauncher.app/Frameworks || exit 1; \
		sudo chown -R 501:501 $(PREFIX)Applications/PocketJLauncher.app/* || exit 1; \
...
package: payload
	...
	echo '[PocketJ Launcher v$(VERSION)] package - end'
	
dsym: payload
	echo '[PocketJ Launcher v$(VERSION)] dsym - start'
	dsymutil --arch arm64 $(OUTPUTDIR)/Payload/PocketJLauncher.app/PocketJLauncher; \
	rm -rf $(OUTPUTDIR)/PocketJLauncher.dSYM; \
	mv $(OUTPUTDIR)/Payload/PocketJLauncher.app/PocketJLauncher.dSYM $(OUTPUTDIR)/PocketJLauncher.dSYM
	echo '[PocketJ Launcher v$(VERSION)] dsym - end'
	
codesign:
	echo '[PocketJ Launcher v$(VERSION)] codesign - start'
	cp '$(PROVISIONING)' $(OUTPUTDIR)/Payload/PocketJLauncher.app/embedded.mobileprovision
	$(call METHOD_MACHO,$(OUTPUTDIR)/Payload/PocketJLauncher.app,$(call METHOD_CODESIGN,$(SIGNING_TEAMID),$$file))
	$(call METHOD_MACHO,$(OUTPUTDIR)/java_runtimes,$(call METHOD_CODESIGN,$(SIGNING_TEAMID),$$file))
	echo '[PocketJ Launcher v$(VERSION)] codesign - end'

clean:
	echo '[PocketJ Launcher v$(VERSION)] clean - start'
	rm -rf $(WORKINGDIR)
	rm -rf JavaApp/build
	rm -rf $(OUTPUTDIR)
	echo '[PocketJ Launcher v$(VERSION)] clean - end'
